...
pub async fn restore_payload_v6_for_raft_replay(
    ...
    restore_payload_v6_with_policy(state, payload, false).await
}

async fn restore_payload_v6_with_policy(
    state: &CoordinatorState,
    payload: BackupPayloadV6,
    keep_local_member_when_absent: bool,
) -> anyhow::Result<()> {
...
pub fn payload_v6_from_json(payload_json: &str) -> anyhow::Result<BackupPayloadV6> {
    ...
    serde_json::from_str(payload_json).context("failed to parse v6 snapshot payload json")
}
