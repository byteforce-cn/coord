...
impl AuthService for AuthGrpc {
    ...
    async fn generate_secret_id(
        ...
        let role_id = if req.role_id.is_empty() {
            ...
            let snapshot = self.security.export_auth_state_snapshot().await;
            snapshot
                .roles
                .into_iter()
    ...
    async fn login_app_role(
        ...
        self.metrics.coord_auth_approle_login_total.inc();
    ...
    async fn lookup_token(
        ...
        let token = self.security.lookup_token(&token_str).await;
