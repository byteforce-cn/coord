...
// ─── RaftProposer port implementation ────────────────────────────────────────

#[async_trait::async_trait]
impl coord_core::proposer::RaftProposer for RaftRuntime {
    async fn propose_business_command_for_result(
        &self,
        namespace: &str,
        payload: Vec<u8>,
    ) -> Result<Vec<u8>, String> {
        self.propose_business_command_for_result(namespace, payload)
            .await
    }

    async fn propose_business_command(
        &self,
        namespace: &str,
        payload: Vec<u8>,
    ) -> Result<(), String> {
        self.propose_business_command(namespace, payload).await
    }
}
