//! `ClientAgent` — 将 Gossip、DiscoveryCache、ProxyClient 组合成统一代理。
//!
...
impl DiscoveryProvider for ClientAgent {
    /// 注册服务实例（广播到 Gossip 环，再通过 CP coord-server 持久化）。
    ///
    /// 当前实现只做 Gossip 广播，后续接入真实 gRPC stub 时补充 CP 路径。
    async fn register(
        &self,
...
mod tests {
    ...
    async fn register_and_discover_via_cache() {
        ...
        a.register(inst("svc-a", "i1"), 60)
            .await
            .expect("register ok");
        // 注册后 cache 被 invalidate，discover 走 gossip（NullAgent 返回空）
    ...
    async fn deregister_removes_from_cache() {
        ...
        // 预填缓存
        cache.put("svc-b", vec![inst("svc-b", "i2")]);
        a.deregister("svc-b", "i2").await.expect("deregister ok");
        let result = a.discover("svc-b").await.expect("discover ok");
        assert!(result.is_empty());
    }
}
