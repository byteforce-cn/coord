...
pub(crate) async fn run(args: ServeArgs, dev_mode: bool) -> anyhow::Result<()> {
    ...
    let workflow_backend = Arc::new(MemoryWorkflowStore::new());
    let workflow_store = Arc::new(RaftWorkflowStore::new(workflow_backend.clone()));
    let workflow_runtime = Arc::new(new_coord_workflow_runtime(workflow_backend.clone()));
