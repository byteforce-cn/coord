# Jepsen evidence — t1.3-scan-60s-kill

| 字段 | 值 |
|:--|:--|
| 场景 | `t1.3-scan-60s-kill` |
| UTC 时间戳 | `20260916T150616Z` |
| run 开始 / 结束 | 2026-09-16 14:47:46,296 / unknown |
...
| 随机种子 | **42** |
| 命令行（jepsen.log 记录） | `lein run test --nodes-file /root/nodes --username root --ssh-private-key /root/.ssh/id_ed25519 --workload scan --nemesis kill --time-limit 60 --concurrency 2n --seed 42` |
...
lein run -m clojure.main scripts/replay.clj store/coord/2026-09-16T14:47:46.271122719Z --seed 42
# 重跑 checker（不需要集群）
lein run -m clojure.main scripts/validate-soak-checker.clj store/coord/2026-09-16T14:47:46.271122719Z
```
