# Jepsen evidence — t1.4-regression-after-f01-f02-fix

| 字段 | 值 |
|:--|:--|
| 场景 | `t1.4-regression-after-f01-f02-fix` |
| UTC 时间戳 | `20260916T150557Z` |
| run 开始 / 结束 | 2026-09-16 14:57:03,656 / unknown |
...
| 随机种子 | **42** |
| 命令行（jepsen.log 记录） | `lein run test --nodes-file /root/nodes --username root --ssh-private-key /root/.ssh/id_ed25519 --workload idempotency --nemesis none --time-limit 45 --concurrency 1n --seed 42` |
...
lein run -m clojure.main scripts/replay.clj store/coord/2026-09-16T14:57:03.631284563Z --seed 42
# 重跑 checker（不需要集群）
lein run -m clojure.main scripts/validate-soak-checker.clj store/coord/2026-09-16T14:57:03.631284563Z
```
