...
> **2026-09-26 更新（第十三轮）**：归档总数已增至 **46 份**。其余 19 份新归档曾停
> 在"待填"，本轮**按参数覆盖面拆分处置**（第十轮备注的裁定口径：回填必须随确认单
> 覆盖面走）：
>
> - **回填 15 份**（同一 tag permalink；跑前 `git diff --quiet` 干净，回填只改该行并
>   重算校验和）：F-05 ×4、idgen/registry ×4、T2.3 浸泡/诊断 ×5、lease
>   pause/partition ×2 —— 这些 run 的参数面属 **①–⑧**；
> - **4 份 m5b-mq 归档保持"待填"**（`20260920T133256Z…` / `…133301Z…` /
>   `…20260921T150133Z…` / `…150135Z…`）：mq 面依赖 **⑨–⑫（lock/cache/mq）尚未确
>   认**的参数（`jepsen/docs/dev.md` §5.4 ⑨–⑫ 均为"待确认"）⇒ 按第十轮裁定**不得**
>   用 ①–⑧ 的确认链接回填（会构成超范围背书）。
>
> 现复核读数：`共 46 份归档：待填 4 / 已回填 39 / 无该字段 3`（无字段者 =
> `20260912T112707Z-java-it`、`20260912T164636Z-round3-workspace-tests`、
> `20260921T161653Z-w6-4-backup-restore-drill`，由另一路径采集）。
>
> **两条仍未签的口径**：③ quiet 门槛（需**引入方团队**签）与 **⑨–⑫**（需处置裁
> 定）；依赖它们的归档（T2.3 浸泡系列 / mq 系列）继续按**内部参考**口径引用。链接
> 指向的 tag 版本自述"③ 未确认"，不存在误导。**回填 ≠ 满签 ≠ 验收通过**（口径不
> 变）。
