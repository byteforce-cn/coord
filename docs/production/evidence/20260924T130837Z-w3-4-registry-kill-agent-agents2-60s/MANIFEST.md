# Jepsen evidence — w3-4-registry-kill-agent-agents2-60s

| 字段 | 值 |
|:--|:--|
| 场景 | `w3-4-registry-kill-agent-agents2-60s` |
| UTC 时间戳 | `20260924T130837Z` |
| run 开始 / 结束 | 2026-09-24 13:00:53,530 / unknown |
...
| 随机种子 | **109486512** |
| 命令行（jepsen.log 记录） | `lein run test --nodes-file /root/nodes --username root --ssh-private-key /root/.ssh/id_ed25519 --workload registry --nemesis kill-agent --time-limit 60 --concurrency 2n --agents 2 --registry-min-cycles 1` |
...
lein run -m clojure.main scripts/replay.clj store/coord/2026-09-24T13:00:53.505550465Z --seed 109486512
# 重跑 checker（不需要集群）
lein run -m clojure.main scripts/validate-soak-checker.clj store/coord/2026-09-24T13:00:53.505550465Z
```
