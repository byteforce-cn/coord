# Jepsen evidence — t1.4-idempotency

| 字段 | 值 |
|:--|:--|
| 场景 | `t1.4-idempotency` |
| UTC 时间戳 | `20260916T132103Z` |
| run 开始 / 结束 | 2026-09-16 13:06:49,052 / unknown |
...
| 随机种子 | **1727727433** |
| 命令行（jepsen.log 记录） | `lein run test --nodes-file /root/nodes --username root --ssh-private-key /root/.ssh/id_ed25519 --workload idempotency --nemesis none --time-limit 45 --concurrency 1n` |
...
lein run -m clojure.main scripts/replay.clj store/coord/2026-09-16T13:06:49.010928736Z --seed 1727727433
# 重跑 checker（不需要集群）
lein run -m clojure.main scripts/validate-soak-checker.clj store/coord/2026-09-16T13:06:49.010928736Z
```
