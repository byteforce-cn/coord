...
| **W6-4** | 备份恢复演练 | 🟡 **本轮先做可复现的进程内演练 + 归档**（见右） | 见 `docs/production/evidence/<ts>-w6-4-backup-restore-drill/` | 跑真实用例（`snapshot_rpc_test` / `snapshot_transfer_test` / `m0_recovery_suite`）并归档 `run.log` + `sha256sums.txt` + MANIFEST。**边界照写不误**：这是**进程内**演练，不含对象存储（`storage` 的 chunk rebuild 边界见 runbook §3.3 与 `adopter-playbook.md` §3）；多节点备份恢复演练 ⏳ 未做 |
